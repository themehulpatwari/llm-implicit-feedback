...
from important_features_paper import PAIRWISE_GAZE_IMPORTANT
...
# Select only gaze important features + text columns + query_id + target + domain
columns_to_keep = text_columns + ['query_id', 'domain'] + PAIRWISE_GAZE_IMPORTANT + [target_column]

# Only keep columns that exist in the dataframe
columns_to_keep = [col for col in columns_to_keep if col in df.columns]
missing_features = [col for col in PAIRWISE_GAZE_IMPORTANT if col not in df.columns]
...
print(f"  Gaze important features: {len([c for c in columns_to_keep if c in PAIRWISE_GAZE_IMPORTANT])}")
print(f"  Target: 1")
...
output_path = Path(__file__).resolve().parent.parent.parent / 'pairwise_output' / 'gaze_important_features_pairwise.csv'
output_path.parent.mkdir(exist_ok=True)
df.to_csv(output_path, index=False)

print(f"\nPairwise gaze important features CSV created")
