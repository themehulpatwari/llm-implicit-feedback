...
input_path = Path(__file__).resolve().parent.parent.parent / 'output' / 'base+extracted+all_metrics+bert.csv'
...
# Merge with oof_base_pairwise.csv for class_1_prob
# print("\nMerging with oof_base_pairwise for class_1_prob...")
...
# df = df.merge(oof_df, on='query_id', how='left')
# print(f"   After merging with oof_base_pairwise: {df.shape}")
...
output_path = Path(__file__).resolve().parent.parent.parent / 'pairwise_output' / 'base+extracted+all_metrics+bert_pairwise.csv'
