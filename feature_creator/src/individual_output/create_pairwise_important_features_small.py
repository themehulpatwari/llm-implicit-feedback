...
# Sample 124 rows with binary_preference=1 and 124 rows with binary_preference=0
df_pos = df[df['binary_preference'] == 1].sample(n=124, random_state=43)
df_neg = df[df['binary_preference'] == 0].sample(n=124, random_state=43)
df = pd.concat([df_pos, df_neg]).reset_index(drop=True)
print(f"\nAfter sampling: {df.shape}")
print(df['binary_preference'].value_counts())

# Save to pairwise_output folder
output_path = Path(__file__).resolve().parent.parent.parent / 'pairwise_output' / 'new_important_features_pairwise_small.csv'
