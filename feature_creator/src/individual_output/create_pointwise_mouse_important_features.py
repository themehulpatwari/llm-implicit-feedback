...
from important_features_old import POINTWISE_MOUSE_IMPORTANT
...
# Select only mouse important features + text columns + user_id + query_id + target + domain
columns_to_keep = text_columns + ['user_id', 'query_id', 'domain', 'preference'] + POINTWISE_MOUSE_IMPORTANT + [target_column]

# Only keep columns that exist in the dataframe
columns_to_keep = [col for col in columns_to_keep if col in df.columns]
missing_features = [col for col in POINTWISE_MOUSE_IMPORTANT if col not in df.columns]
...
print(f"  Mouse important features: {len([c for c in columns_to_keep if c in POINTWISE_MOUSE_IMPORTANT])}")
print(f"  Target: 1")
...
output_path = Path(__file__).resolve().parent.parent.parent / 'pointwise_output' / 'mouse_important_features_pointwise.csv'
output_path.parent.mkdir(exist_ok=True)
df.to_csv(output_path, index=False)

print(f"\nPointwise mouse important features CSV created")
