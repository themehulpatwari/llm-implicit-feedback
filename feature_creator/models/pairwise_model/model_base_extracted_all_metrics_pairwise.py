...
Dataset: base+extracted+all_metrics (combines extracted features + additional metrics)
...
CONFIG = {
    ...
    # Data paths
    'INPUT_CSV': 'pairwise_output/base+extracted+all_metrics_pairwise.csv',
...
def train_and_evaluate_model(X, y, config):
    ...
    output_file = f'{output_dir}/importance_base_extracted_all_metrics_pairwise.csv'
...
def main():
    ...
    # Load original dataframe (for augmented output) - apply same NA-dropping logic
    df_original = pd.read_csv(CONFIG['INPUT_CSV'])
    na_cols = [col for col in df_original.columns if df_original[col].isna().all()]
    if na_cols:
        df_original = df_original.drop(columns=na_cols)
    ...
    # Generate OOF labels
    generate_oof_labels(X, y, df_original, CONFIG, 'base_extracted_all_metrics_pairwise_labeled.csv')
