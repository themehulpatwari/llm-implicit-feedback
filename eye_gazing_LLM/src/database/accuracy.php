...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    $accuracy = (int) filter_var($data['accuracy'], FILTER_SANITIZE_NUMBER_INT); 
    ...
    if($connection->connect_error){
        ...
        $stmt = $connection->prepare("UPDATE $table SET accuracy = ? WHERE user_id = ?;");
        if(!$stmt){
            responseFailure(500, "Prepare Statement Failed");
        }

        if(!$stmt->bind_param("is", $accuracy, $user_id)){
