...
for idx, qid in enumerate(query_order):
    ...
    if gpts:
        ...
        # ── Saccade lines ─────────────────────────────────────────────────────
        long_jump = 15 * char_w    # skip return-sweeps (≥15 char widths apart)
        fwd_segs, reg_segs = [], []
        for i in range(len(xs) - 1):
            ...
            dy  = ys[i+1] - ys[i]
            dx  = xs[i+1] - xs[i]
            is_fwd = (dy < -line_h * 0.4) or (abs(dy) < line_h * 0.6 and dx >= 0)
            (fwd_segs if is_fwd else reg_segs).append(seg)

        if fwd_segs:
            ax_t.add_collection(LineCollection(
                fwd_segs, colors='#4cc9f0', linewidths=0.7, alpha=0.45, zorder=3))
        if reg_segs:
            ax_t.add_collection(LineCollection(
                reg_segs, colors='#ff9f1c', linewidths=0.9, alpha=0.55, zorder=3))
        ...
        # Legend
        hdl = [
            mpatches.Patch(color='#4cc9f0', label='Forward saccade'),
            mpatches.Patch(color='#ff9f1c', label='Regression'),
