...
def preference_loss(policy_chosen_logps: torch.FloatTensor,
                    ...
                    ipo: bool = False, reference_free: bool = False,
                    rdpo=False, epsilon=None) -> Tuple[torch.FloatTensor, torch.FloatTensor, torch.FloatTensor]:
    ...
    elif rdpo: # https://arxiv.org/pdf/2403.00409
        assert epsilon is not None
        l_chosen   = -F.logsigmoid( beta * logits)  # L(θ; ỹ_w, ỹ_l)
        l_rejected = -F.logsigmoid(-beta * logits)  # L(θ; ỹ_l, ỹ_w)
        losses = ((1 - epsilon) * l_chosen - epsilon * l_rejected) / (1 - 2 * epsilon)
    else:
...
class BasicTrainer(object):
    def __init__(self, policy: nn.Module, config: DictConfig, seed: int, run_dir: str, reference_model: Optional[nn.Module] = None, rank: int = 0, world_size: int = 1):
        ...
        data_iterator_kwargs = dict(
            data_path_name=config.data_path_name,
    ...
    def get_batch_metrics(self, batch: Dict[str, Union[List, torch.LongTensor]], loss_config: DictConfig, train=True):
        ...
        if loss_config.name in {'dpo', 'ipo', 'rdpo'}:
            ...
            elif loss_config.name == 'rdpo':
                epsilon = 1.0 - torch.tensor(batch['confidence'], dtype=torch.float32, device=policy_chosen_logps.device)
                epsilon = epsilon.clamp(0.0, 0.45)
                loss_kwargs = {'beta': loss_config.beta, 'reference_free': loss_config.reference_free, 'ipo': False, 'rdpo': True, 'epsilon': epsilon}
            ...
            if loss_config.alpha > 0:
                ...
                losses = losses + loss_config.alpha * nll_loss
                nll_loss_gathered = all_gather_if_needed(nll_loss.detach(), self.rank, self.world_size)
                metrics[f'loss_{train_test}/nll'] = nll_loss_gathered.cpu().numpy().tolist()
        ...
        for batch in self.train_iterator:
            ...
            if self.example_counter % self.config.eval_every == 0 and (self.example_counter > 0 or self.config.do_first_eval):
                ...
                if self.example_counter > 0:
                    ...
                    elif self.config.save_during_eval:
            ...
            #### END TRAINING ####
        #if not self.config.save_during_eval:
        #    output_dir = os.path.join(self.run_dir, f'step-{self.example_counter}')
        #    rank0_print(f'creating checkpoint to write to {output_dir}...')
        #    self.save(output_dir, mean_eval_metrics)
...
class FSDPTrainer(BasicTrainer):
    def __init__(self, policy: nn.Module, config: DictConfig, seed: int, run_dir: str, reference_model: Optional[nn.Module] = None, rank: int = 0, world_size: int = 1):
        ...
        if config.loss.name in {'dpo', 'ipo', 'rdpo'}:
