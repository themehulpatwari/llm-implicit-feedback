...
def get_eye_pair(data_path_name: str, split: str, silent: bool = False, cache_dir: str = None, sft_rej: bool=False) -> Dict[str, Dict[str, Union[List[Tuple[int, int]], List[str], str]]]:
    print(f'Loading eye gazing pairwise dataset ({split} split)...')
    #dataset_path = f"data/base_pairwise_{split}.csv"
    dataset_path = f"{data_path_name}_{split}.csv"
    df = pd.read_csv(dataset_path)
    print('done')

    def get_chosen(r1, r2, label):
        if label == 1:
            #return r2, r1
            return r1, r2
        elif label == 0:
    ...
    for index, row in tqdm.tqdm(df.iterrows(), desc='Processing Eye Gazing', disable=silent):
        if row['binary_preference'] != 0 and row['binary_preference'] != 1:
            print('no labels')
            continue
        prompt = "\n\nHuman: "+row['user_query']+"\n\nAssistant:"
        chosen, rejected = get_chosen(row['llm_response_1'], row['llm_response_2'], row['binary_preference'])
        ...
        if 'confidence' in row:
            ...
            data[prompt]['confidence'] = confidence
        if sft_rej:
            data[prompt]['sft_target'] = rejected
        else:
            data[prompt]['sft_target'] = chosen

    return data

def get_hh_single(split: str, silent: bool = False, cache_dir: str = None) -> Dict[str, Dict[str, Union[List[Tuple[int, int]], List[str], str]]]:
...
def get_dataset(data_path_name: str, name: str, split: str, silent: bool = False, cache_dir: str = None):
    ...
    if name == 'shp':
        ...
        data = get_hh_single(split, silent=silent, cache_dir=cache_dir)
    elif name == 'eyep':
        data = get_eye_pair(data_path_name, split, silent=silent, cache_dir=cache_dir, sft_rej=False)
    elif name == 'eyep_r':
        data = get_eye_pair(data_path_name, split, silent=silent, cache_dir=cache_dir, sft_rej=True)
    elif name == 'se':
    ...
    for prompt in data:
        if 'confidence' not in data[prompt]:
            data[prompt]['confidence'] = 1
    
    assert set(list(data.values())[0].keys()) == {'responses', 'pairs', 'sft_target', 'confidence'}, \
...
def get_batch_iterator(names: List[str],
    ...
    with TemporarilySeededRandom(seed):
        ...
        for name in names:
            ...
            for prompt, data in get_dataset(data_path_name, name, split, silent=silent, cache_dir=cache_dir).items():
                flat_data.append((prompt, data['responses'], data['pairs'], data['sft_target'], data['confidence'], truncation_mode))
    ...
    while True:
        ...
        batch = []
        for prompt, responses, pairs, sft_target, confidence, truncation_mode in flat_data:
            if done:
                break
            if sft_mode:
                ...
                for p in pairs:
                    ...
                    batch_element['confidence'] = confidence
                    batch.append(batch_element)
                    example_idx += 1
                    if len(batch) == batch_size:
                        yield collate_fn(batch)
